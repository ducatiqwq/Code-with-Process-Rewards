...
# ── CONFIG ──────────────────────────────────────────────────────────────────────
MODEL1_PATH = "/root/autodl-tmp/ckpt/ours/sftv2/checkpoint-2640"
# MODEL2_PATH = "Qwen/Qwen2.5-1.5B-Instruct"
MODEL2_PATH = "/home/ducati/projects/finalproject/rllm/checkpoints/deepcoder/1.5b-ours-cot-nodoc/actor/global_step_32"
JSON_DATA   = "/home/ducati/projects/finalproject/rllm/data/baseline_train.json"
# JSON_DATA   = "/home/ducati/projects/finalproject/rllm/data/test_livecodebench.json"
OUT_PATH    = "/root/autodl-tmp/outputs/eval_train_output_ours_nodoc_sftv2.json"
BATCH_SIZE  = 32
...
def get_prompt_stage2(input_prompt, stage1_output):
	problem = re.search(r'passes all tests\.(.*?)### Format', input_prompt, re.DOTALL)
	if problem:
		problem = problem.group(1).strip()
	else:
		raise ValueError("Could not find problem description in the input prompt.")
	...
	return get_stage2_prompt(problem, incomplete_code)
...
def main():
	...
	#for testing, only load the first 5 data samples
	...
	def stage1_worker():
		for batch_idx, orig_prompts in enumerate(loader):
			...
			s1_prompts = [get_prompt_stage1(p[0]['content']) for p in orig_prompts]
			...
			# delete all docstrings of s1_outs
			s1_outs = [re.sub(r'    \"\"\".*?\"\"\"\n', '', out, flags=re.DOTALL) for out in s1_outs]
	...
	def stage2_worker():
		while True:
			...
			s2_prompts = [get_prompt_stage2(orig_prompt[0]['content'], o) for (orig_prompt, o) in zip(orig_prompts, s1_outs)]
			for s2_prompt in s2_prompts:
				print("=" * 50)
				print(s2_prompt)
				print("=" * 50 + "\n\n")
			with AUTOCAST(dtype=torch.bfloat16):
				...
				out2_ids = model2.generate(**enc2, max_new_tokens=4096, temperature=0.1)
				# remove input part
...
def compute_acc():
	...
	with open(JSON_DATA, "r", encoding="utf-8") as f:
		samples = json.load(f)
		samples = samples[:len(preds)]  # ensure same length

	assert len(preds) == len(samples), "Number of predictions and ground truths do not match."
	...
	for pred, sample in zip(preds, samples):
		...
		try:
			...
			print("=" * 50)
			print(code)
			print(f"Verdict: {verdict}, Metadata: {metadata}")
			print("=" * 50 + "\n\n")
			if verdict:
				num_correct += 1
		except Exception as e:
			print(f"Error processing sample: {sample['prompt']}, error: {e}")
			continue
